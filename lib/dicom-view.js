...
class DicomView extends PolymerElement {
  ...
  upd_state () {
    window.states[cornerstone.getImage(this.$.dicomImage).imageId] = cornerstoneTools.getToolState( this.$.dicomImage , 'FreehandMouse');
    // console.log(x)
    // var zip = new JSZip();
    // for (var i = 0; i < 5; i++) {
    //     var txt = 'hello';
  ...
  _imageIdObserver(imageId) {
    ...
    var old_image =  cornerstone.getImage(this.$.dicomImage)

    console.log(old_image)
      if (old_image  !== undefined ) {
        window.states[old_image.imageId] = cornerstoneTools.getToolState( this.$.dicomImage , 'FreehandMouse');
      }
