...
class PageDicom extends PolymerElement {
  ...
  connectedCallback() {
    ...
    fetch(`/dicom/api/study/get?id=${this.location.params.id}`)
      .then(response => response.json())
      .then((data) => {
        data.files = data.files.reverse()
        data.files.forEach(file => {

          fetch(`/dicom/api/study/getFile`,
            {
              method: 'POST',
              headers: {
        ...
        // fetch(`/dicom/api/study/getFile?id=${this.location.params.id}`)
        //   .then(response => response.json())
        //   .then((data) => {
        //     this.set('file', data);
        //   });
...
customElements.define('page-dicom', PageDicom);
